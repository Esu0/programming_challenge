...
use crate::segtree::{MaxQuery, SegTree};
...
mod segtree {
    ...
    impl<T> SegTree<T> {
        pub fn iter(&self) -> std::iter::Skip<std::slice::Iter<'_, T>> {
            self.into_iter()
        }
    }
...
fn main() {
    ...
    let w_scan = (0..n).map(|i| (i, scan!(u16)));
    // let mut w = (0..n).map(|i| (i, scan!(u16))).collect::<Vec<_>>();
    let mut bin = [None; 10000];
    for (i, wi) in w_scan {
        bin[wi as usize] = Some(i as i16);
    }
    let mut bin2 = unsafe { MaybeUninit::<[MaybeUninit<u16>; 2001]>::uninit().assume_init() };
    for (w, i) in bin.into_iter().flatten().enumerate() {
        bin2[i as usize].write(w as u16);
    }
    // w.sort_unstable_by_key(|&(_, w)| w);
    // w.iter_mut()
    //     .enumerate()
    //     .for_each(|(i, (_, w))| *w = i as u16);
    // w.sort_unstable_by_key(|&(i, _)| i);
    // println!("{:?}", bin2);
    let w_ptr = &bin2[..n as usize] as *const _ as *const [u16];
    let w = unsafe { &*w_ptr };
    let mut dp1 = SegTree::from_iter(iter::repeat(MaxQuery(0u32)).take(n as usize));
    for &wi in w.iter().rev() {
        let v1 = dp1.query(..wi as usize).0 + 1;
        dp1.update(wi as usize, &MaxQuery(v1));
    }

    let mut dp2 = SegTree::from_iter(iter::repeat(MaxQuery(0u32)).take(n as usize));
    for &wi in w.iter().rev() {
        let v2 = dp2.query((wi as usize + 1)..).0 + 1;
        dp2.update(wi as usize, &MaxQuery(v2));
    }

    let ans = dp1.iter().zip(&dp2).map(|(a, b)| a.0 + b.0).max().unwrap() - 1;
    println!("{ans}");
}
